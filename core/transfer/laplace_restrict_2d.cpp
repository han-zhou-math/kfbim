    ...
    const std::vector<LocalPoly2D>& correction_polys) const
{
    ...
    if (iface.points_per_panel() != 3
        || iface.panel_node_layout() != PanelNodeLayout2D::LegacyGaussLegendre) {
        throw std::invalid_argument("LaplaceQuadraticRestrict2D is legacy and requires Gauss-Legendre 3-point panels");
    }
        ...
        if (grid_pair_.domain_label(idx) == 0) {
            const int center_idx =
                nearest_center_for_grid_node(grid, center_polys,
                                             ...
                                             idx);
            val += evaluate_taylor_poly_2d(center_polys[center_idx], pt);
        }
        rhs[r] = val;
    }
