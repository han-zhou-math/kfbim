...
namespace kfbim_test_ellipsoid_3d {

constexpr double kEllipsoidCx = 0.07;
constexpr double kEllipsoidCy = -0.04;
constexpr double kEllipsoidCz = 0.03;
constexpr double kEllipsoidAxisA = 0.61;
constexpr double kEllipsoidAxisB = 0.49;
...
inline Eigen::Vector3d ellipsoid_center()
{
    return {kEllipsoidCx, kEllipsoidCy, kEllipsoidCz};
}

inline double max_ellipsoid_axis()
{
    return std::max({kEllipsoidAxisA, kEllipsoidAxisB, kEllipsoidAxisC});
}
...
inline Eigen::Vector3d ellipsoid_point(Eigen::Vector3d unit)
{
    unit.normalize();
    return {kEllipsoidCx + kEllipsoidAxisA * unit[0],
            kEllipsoidCy + kEllipsoidAxisB * unit[1],
            kEllipsoidCz + kEllipsoidAxisC * unit[2]};
}

inline Eigen::Vector3d ellipsoid_normal(Eigen::Vector3d unit)
{
    unit.normalize();
    Eigen::Vector3d normal(unit[0] / kEllipsoidAxisA,
                           unit[1] / kEllipsoidAxisB,
                           unit[2] / kEllipsoidAxisC);
    normal.normalize();
    return normal;
}

inline int add_ellipsoid_node(EllipsoidMeshData& mesh, Eigen::Vector3d unit)
{
    unit.normalize();
    ...
    mesh.directions.push_back(unit);
    mesh.points.push_back(ellipsoid_point(unit));
    mesh.normals.push_back(ellipsoid_normal(unit));
    mesh.weights.push_back(0.0);
    return idx;
}

inline int add_edge_midpoint(EllipsoidMeshData& mesh, int a, int b)
{
    if (a > b)
        std::swap(a, b);
    ...
    Eigen::Vector3d unit = mesh.directions[a] + mesh.directions[b];
    unit.normalize();
    const int idx = static_cast<int>(mesh.points.size());
...
inline kfbim::Interface3D make_p2_ellipsoid(int subdivision)
{
    ...
    for (const auto& face : base_faces) {
        ...
        for (int i = 0; i <= subdivision; ++i) {
            ...
            for (int j = 0; j <= subdivision - i; ++j)
                ids[i][j] = add_ellipsoid_node(mesh, lattice_unit(face, i, j));
        }
...
inline int ellipsoid_surface_subdivision_for_grid(int N)
{
    const double approx_subdivision =
        kOctahedronEdgeLengthOnUnitSphere * max_ellipsoid_axis()
