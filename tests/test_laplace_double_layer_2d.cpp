// 2nd kind BIE for Dirichlet BVP using double-layer representation.
//
// Formulation:
//   u = u_part + D[phi]
// where:
...
//
// Interior BVP: (K - 1/2 I) phi = g - u_part|Γ   (DirichletDouble mode)
// Exterior BVP: (K + 1/2 I) phi = g - u_part|Γ   (ExteriorDirichletDouble mode)
//
...
static double run_double_layer_dirichlet(int N, LaplaceKFBIMode mode)
{
    ...
    op.apply_full(phi, u_bulk); // Placeholder, apply_full returns trace, not bulk u
    
    // To get bulk u, we need to manually redo the solve (or add a method for it)
    ...
    int target_label = (mode == LaplaceKFBIMode::DirichletDouble) ? 1 : 0;
    double err = 0.0;
          ...
          "[kfbi][laplace][double_layer][interior][2d]")
{
    ...
    double errs[3];
    for (int i = 0; i < 3; ++i) {
        errs[i] = run_double_layer_dirichlet(Ns[i], LaplaceKFBIMode::DirichletDouble);
...
TEST_CASE("Laplace 2nd kind BIE (Double Layer): Exterior Dirichlet BVP",
          "[kfbi][laplace][double_layer][exterior][2d]")
{
    std::printf("\n  Exterior Dirichlet BVP (Double Layer, 2nd kind BIE):\n");
    const int Ns[] = {32, 64, 128};
    double errs[3];
    for (int i = 0; i < 3; ++i) {
        errs[i] = run_double_layer_dirichlet(Ns[i], LaplaceKFBIMode::ExteriorDirichletDouble);
        if (i == 0) std::printf("    N=%3d  err=%.4e\n", Ns[i], errs[i]);
        else {
            double rate = std::log2(errs[i-1] / errs[i]);
            std::printf("    N=%3d  err=%.4e  rate=%.2f\n", Ns[i], errs[i], rate);
