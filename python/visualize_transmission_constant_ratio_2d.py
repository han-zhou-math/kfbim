#!/usr/bin/env python3
"""Create Python visualizations for the constant-ratio transmission test."""
...
def load_interface_points(csv_path: Path):
    iface_path = csv_path.with_name(
        csv_path.name.replace(
            "transmission_constant_ratio_N",
            "transmission_constant_ratio_interface_N",
        )
    )
    if not iface_path.is_file():
        return None

    data = np.genfromtxt(iface_path, delimiter=",", names=True)
    if data.size == 0:
        return None
    data = np.atleast_1d(data)
...
def plot_convergence(out_dir: Path) -> None:
    ...
    ax.set_title("Constant-ratio transmission convergence")
...
def plot_grid(csv_path: Path) -> None:
    ...
    iface_points = load_interface_points(csv_path)
    n_label = csv_path.stem.split("_")[-1]

    fig, axes = plt.subplots(1, 3, figsize=(13.0, 4.0), constrained_layout=True)
    panels = [
        ("u_bulk", grid["u_bulk"], "viridis"),
        ("abs_error", grid["abs_error"], "magma"),
    ...
    fig.suptitle(f"Constant-ratio transmission, {n_label}")
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("usage: visualize_transmission_constant_ratio_2d.py OUTPUT_DIR", file=sys.stderr)
        return 2
    ...
    for csv_path in sorted(out_dir.glob("transmission_constant_ratio_N*.csv"), key=level_key):
        plot_grid(csv_path)
    return 0
