    ...
    for (int p = 0; p < iface.num_panels(); ++p) {
        for (int t = 0; t < centers_per_panel; ++t) {
            ...
            for (int q = 0; q < 6; ++q) {
                ...
                bdry1[q][1] = pt[1];
                bdry1[q][2] = pt[2];
                av[q] = detail3d::panel_scalar(iface, p, a, dir_bary[q]);
            ...
            for (int q = 0; q < 3; ++q) {
                ...
                bv[q] = detail3d::panel_scalar(iface, p, b, neu_bary[q]);
            }

            const double bulk[3] = {center[0], center[1], center[2]};
            const double Lu = detail3d::panel_scalar(iface, p, Lu_iface, center_bary);
